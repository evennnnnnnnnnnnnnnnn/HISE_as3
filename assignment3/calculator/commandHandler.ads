...
package commandHandler with SPARK_Mode is
   ...
   procedure Handle_Lock(Cmd : in Command;
     ...
     Pre => Get_Cmd(Cmd) in Unlock | Lock and Master_Str'Length = 4,
     ...
     (if Get_Cmd(Cmd) = Lock and then State'Old = True and then
       ...
       (for all C of To_String(Get_Arg1(Cmd)) => C in '0' .. '9') then
         State = False
         and then Master_Pin = From_String(Master_Str))
      and
      -- State remains unchanged if locking when already locked
      (if Get_Cmd(Cmd) = Lock and then State'Old = False then State = State'Old);


   procedure Handle_Stack(Cmd : in Command; S : in out Stack_type) with
     Pre => Get_Cmd(Cmd) in Push1 | Push2 | Pop,
   ...
   procedure Handle_Arithmetic(Cmd : in Command; S : in out Stack_type) with
     Pre => Get_Cmd(Cmd) in Add | Sub | Mul | Div,
